...
// For mobile, we need the actual IP address or a tunnel if testing on a device.
// Defaulting to localhost:8080 for emulator.
const BASE = 'http://192.168.1.40:9695/api'; // 10.0.2.2 is the address to access host localhost in Android emulator

const getAuthHeader = async () => {
  const token = await AsyncStorage.getItem('fintrack_token');
...
async function request<T>(url: string, options?: RequestInit): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    // We should ideally trigger a logout in the context here
...
export const accountsApi = {
  ...
  get: (id: number, params?: { date_before?: string; date_to?: string }) => {
    ...
    if (params) {
      ...
      if (params.date_before) search.append('date_before', params.date_before);
...
export const categoriesApi = {
  list: (params?: { entity?: string; nature?: string }) => {
    const search = new URLSearchParams();
    if (params?.entity) search.append('entity', params.entity);
    if (params?.nature) search.append('nature', params.nature);
    const qs = search.toString();
